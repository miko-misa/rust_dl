...
pub trait LossFunction {
  fn forward(&self, input: ArrayD<f64>, target: ArrayD<f64>) -> f64;
  fn backward(&self, input: ArrayD<f64>, target: ArrayD<f64>) -> ArrayD<f64>;
}
...
impl LossFunction for CrossEntropyLoss {
  fn forward(&self, input: ArrayD<f64>, target: ArrayD<f64>) -> f64 {
    let esp = 1e-15;
    let y_pred = input.view().into_dimensionality::<Ix2>().unwrap();
