...
#table(
  ...
  [$tensor(X)$],[],[テンソル（大文字・太文字・カリグラフィー）],
  [$x_(a_1a_2 dots a_n)$],[],[テンソル$bold(X)$の$a_1,a_2,dots,a_n$成分],
  [$bold(x)_(a_1a_2 dots a_(n-1)*)$],[],[$(x_(a_1a_2 dots 1),x_(a_1a_2 dots 2),dots,x_(a_1a_2 dots a_n))$],
...
全結合層では1つのニューロンは入力すべてと結合するが、局所受容野を持つ層では1つのニューロンは入力の一部（=局所受容野）としか結合しない。あるニューロンの出力$y_j$は局所受容野からの入力を受け取り、次のニューロンは別の局所受容野をスライドしてそこからの入力を受け取る。局所受容野の幅を$f$、局所受容野のスライド幅を$s$とする。このとき、入力の長さが$s$の倍数であることは、暗に仮定しよう。上の図では$(f,s)=(2,1)$である。すると
$
...
$
このとき、重み$w$は単純に$display(f times n/s)$だけ準備することになる。さらにパラメータ数を減らすために全ての局所受容野で同じ重みを用いることにする。この重みは局所受容野の重みと呼ばれ、局所受容野から特徴量を正しく抽出するように学習される。つまり、重み共有を行うと、出力$y_j$は次のように表せる。
$
...
そもそも、画像データとは何次元のデータだろうか。全結合層はバッチを考えなければ、一次元データであるベクトルに対して適応されてきた。特徴量一つが一つの成分を表すベクトルである。すると画像は、ピクセルを縦横に並べた2次元のデータだろうか？一般的にはそうではない。画像は、ピクセルを縦横に並べた2次元の構造に加えて、1ピクセルが色の情報をベクトルで持つ。つまり、画像は3次元のデータである。例えば、RGB画像は、縦横のピクセルに加えて、色の情報を3成分のベクトルで持つ。つまり、画像は画像の横方向、縦方向、そして色方向の3次元である。これをテンソルと呼ぶ。テンソルは行列と同様に大きさが存在し、$a times b times c times dots$と表す。一般的に画像は$H times W times C$と表される。$H$は高さ、$W$は幅、$C$はチャンネル数である。RGB画像は$H times W times 3$で表される。#footnote[チャンネルは色の情報を表すが、チャンネル軸を一番最初の軸として$C times H times W$と表すこともある。本書では$H times W times C$と表す。]グレースケール画像は$H times W times 1$で表される。テンソルは、行列やベクトルを一般化した概念であるが、本書では3次元以上のデータをテンソルと呼ぶことにする。$n$次元実数テンソル全体は$RR^(a_1 times a_2 times dots times a_n)$である。

また、テンソルは添え字を指定することで行列やベクトルとして扱うこともできる。3次元テンソル$tensor(X) in RR^(H times W times C)$に対して、
$
  ...
  dots.v, dots.v, dots.down, dots.v;
  x_(i H 1), x_(i H 2), dots, x_(i H C)
  ) in RR^(H times C)\
$
となる。特に、$tensor(X)$はベクトルの行列と同一視できることは重要である。
$
  tensor(X) &= mat(
    bold(x)_(1 1 *), bold(x)_(1 2 *), dots, bold(x)_(1 C *);
    bold(x)_(2 1*), bold(x)_(2 2*), dots, bold(x)_(2 C*);
    dots.v, dots.v, dots.down, dots.v;
    bold(x)_(H 1*), bold(x)_(H 2*), dots, bold(x)_(H C*)
  )
$

== 畳み込み層
では畳み込みをテンソルに対して適応することを考える。まずは、純粋に拡張してみる。テンソル$tensor(X) in RR^(H times W times C)$に対して畳み込みを適応し、出力$tensor(y) in RR^(H^prime times W^prime times C^prime)$を得ることを考える。カーネルの大きさ（=カーネルサイズ）を$f_H times f_W times f_C$、カーネルのスライド幅をそれぞれの軸について$s_H, s_W, s_C$とする。すると、カーネルの重みは$tensor(W) in RR^(f_H times f_W times f_C)$となる。すると、出力は次のように表せる。
$
...
$
  bold(y)_(i j) &= sum_(1 <= h <= f_H) sum_(1 <= w <= f_W) bold(w)_(h w*) dot.circle bold(x)_(h + s_H times (i - 1), w + s_W times (j - 1),*)\
$
と定式化できる。すると、出力の大きさはどうなるだろうか？ここでもストライドはそれぞれの軸について倍数であることを仮定する。すると、出力の大きさは次のように表せる。
$
  H^prime &= (H - f_H + 1)/s_H\
  W^prime &= (W - f_W + 1)/s_W\
  C^prime &= 1\
$
ここで、$C$方向の大きさが$1$になっていることに着目すると、画像の1ピクセルの特徴量が1つに圧縮されていることがわかる。しかし、これはあまり望ましくない。なぜなら、1ピクセルの表現力が失われてしまうからである。また、この計算は$H,W$方向については畳み込みを、$C$方向についてはすべての成分に対して異なる重みを用いてその総和を計算していることから1ニューロンをもつ全結合層の計算を行っていると解釈できる。つまり、全結合層と同じようにニューロンを増やすことで表現力を増すことができる。そのためには、$C$方向の各成分について異なる重みを準備し、複数回変換を行う必要性ある。よって、異なる$C^prime$個のカーネル$tensor(W)^((1)),tensor(W)^((2)),dots,tensor(W)^((C^prime))$を準備し、個々の出力$tensor(Y)^((1)),tensor(Y)^((2)),dots,tensor(Y)^((C^prime))$を得てみよう。
$
  tensor(Y)^((k))_(i,j) &= sum_(1 <= h <= f_H) sum_(1 <= w <= f_W) tensor(W)^((k))_(h w*) dot.circle bold(x)_(h + s_H times (i - 1), w + s_W times (j - 1),*)
$
そして、この個々の出力は$C$方向が$1$のテンソルであり、これは$2$次元テンソルつまりは行列として扱える。これを失われた$C$方向に結合することで、真の出力$tensor(Y)$を得ることができる。
$
  tensor(Y) &= (tensor(Y)^((1)), tensor(Y)^((2)), dots, tensor(Y)^((C^prime)))\
  tensor(Y)_(i,j,k) &= sum_(1 <= h <= f_H) sum_(1 <= w <= f_W) tensor(W)^((k))_(h w*) dot.circle bold(x)_(h + s_H times (i - 1), w + s_W times (j - 1),*)
$
このような計算により、入力テンソル$RR^(H times W times C)$に対して、出力テンソル$RR^(H^prime times W^prime times C^prime)$を得ることができる。$C^prime$は何枚のカーネルを用いるかを与えることになり、モデル設計の段階で決定する。ちょうど、全結合層における出力の次元数を決定するのと同じである。なので、ここで改めて入力チャンネル数$C_text("in")$と出力チャンネル数$C_text("out")$を定義する。また、入力$tensor(X)$と重み$tensor(W)^((k))$のストライド$(s_H,s_W)$による畳み込みを以下のように表すこととする。
$
  tensor(Y)^((k)) &= tensor(X) attach(ast.op, b: (s_H,s_W)) tensor(W)^((k))\
$

さらに、バイアスについて考えてみよう。バイアスは、ニューロン1つが1つの値を持ち、入力を重みでスケールさせたのちに加えるスカラーであった。つまり、先ほどの出力$tensor(W)^((k))$の全要素に同一のバイアス$b_k$を加えればよい。改めてその出力は、
$
  tensor(X)&in& RR^(H times W times C_text("in")):& text("input") \
  tensor(W)&in& RR^(f_H times f_W times C_text("in")):& text("weight") \
  tensor(Y)&in& RR^(H^prime times W^prime times C_text("out")):& text("output") 
$
#v(-2em)
$
